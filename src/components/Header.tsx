    ...
    <header className="sticky top-0 w-full z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200/80 shadow-xs">
      <div className="flex justify-between items-center px-4 md:px-8 max-w-[1280px] mx-auto h-20">
        <div className="flex items-center gap-4">
          ...
          >
            <div className="relative p-2 rounded-2xl bg-gradient-to-br from-sky-500 via-blue-600 to-cyan-600 shadow-md shadow-sky-500/20 group-hover:scale-105 transition-all">
              <img
            ...
            <div className="flex flex-col">
              <span className="font-extrabold text-xl md:text-2xl text-slate-900 tracking-tight flex items-center gap-1.5">
                ITパスポート <span className="bg-gradient-to-r from-sky-600 via-blue-600 to-cyan-600 bg-clip-text text-transparent">LAB</span>
              </span>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 text-sm font-semibold py-2 px-4 rounded-xl transition-all cursor-pointer ${
                  isActive
                    ? 'bg-white text-sky-600 shadow-sm border border-slate-200/80 font-bold'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
                }`}
              >
                <span className={`material-symbols-outlined text-lg ${isActive ? 'text-sky-600' : 'text-slate-400'}`}>
                  {tab.icon}
                </span>
                {tab.label}
              </button>
        ...
        <div className="flex items-center">
          <form onSubmit={handleSearchSubmit} className="relative hidden sm:block w-60 lg:w-72">
            <input
              ...
              className="w-full bg-slate-100/90 border border-slate-200 rounded-2xl py-2 pl-10 pr-4 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all shadow-inner"
            />
